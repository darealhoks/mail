# map

- [store.md](store.md) — db schema, who reads/writes what
- [sources.md](sources.md) — source module contract + registry; bakalari + teams protocol facts
- [auth-ux.md](auth-ux.md) — teams session death handling, resign flow ux
- [config.md](config.md) — config file sections and keys, flags, binds, school-year periods

## layout

    src/core/     store config creds json text http oauth notify term  — no source, no ui
    src/sources/  bakalari teams teams_auth outlook classify + registry.h — one pair per source
    src/view/     store -> row structs, pure: no printf, no ansi, no term width
                  + paint.cpp: the renderers themselves — feed posts, marks rows, timetable grid, sgr helpers
    src/maild/    main.cpp     oneshot fetch + notify; run from cron: */15 * * * * maild
    src/cli/      cli.cpp      argv, binds, layout, paint
    src/tui/      tui.cpp      raw-mode feed/marks/timetable/absence, tab strip, keys + mouse, no flags
    src/tests/    main.cpp     `make check`; not built by `make`, not installed

`make` builds all three binaries from `src/core src/sources src/view` plus one main each.
`make check` builds and runs `build/mail-check`; `make corpus` scores the classifier against
`tests/corpus.tsv`, whose last column is the expected kind.

`src/core/text.cpp` is the shared text layer — html to plain, unescape, collapse, invisible
strip, utc format, first-line titles. It exists so no source has to include another source
and no view has to include a source.

## cli verbs

    (none)          the feed, most urgent last; bare words are filters
    auth [src]      sign-in state, or sign into one source
    new             one line for the shell prompt hook; prints, never prompts
    marks [filter]  marks by period, with per-subject absence under it
    absence, b      absence per subject; bare words filter by subject
    timetable       the week's grid; +n/-n or YYYY-MM-DD picks another week
    next [--help]   the next lesson; -f/-s format it, --help lists the % verbs
    dismiss / open  by feed index
    help, --version

Aliases are one letter (`a n m r d o b`; `next` has none — `n` is `new`) and any of them can be
taken over by a `[bind]` entry.
